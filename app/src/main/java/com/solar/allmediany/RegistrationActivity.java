...
import static com.solar.allmediany.R.id.editUname;

public class RegistrationActivity extends Activity {
	
	ImageView topLeftBtn;
	EditText tvPassword, tvUsername, tvEmail, tvFname, tvLname, tvAddr, tvZip, tvPhone;
	TextView titlename, tvDOB, tvCountry, tvState, tvCity;
	Button btnEditProf;
	...
	public String strUserPwd = "", strUsername = "", strFname = "", strLname = "", strEmail = "", 
			strZip = "", strCity = "", strCityId = "", strState = "", strStateId = "", strStateShortname = "", 
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btnEditProf.setOnClickListener(new OnClickListener() {			
			...
			public void onClick(View v) {
				...
				imm.hideSoftInputFromWindow(tvUsername.getWindowToken(), InputMethodManager.HIDE_NOT_ALWAYS);

				strUsername	= tvUsername.getText().toString();
				strUserPwd	= tvPassword.getText().toString();
				...
				if(null==strUsername || strUsername.length()==0 || strUsername.equalsIgnoreCase(" ")){
					Toast.makeText(getApplicationContext(),"Please enter Username",Toast.LENGTH_LONG).show();
				}
				else if(null==strUserPwd || strUserPwd.length()==0 || strUserPwd.equalsIgnoreCase(" ")){
					Toast.makeText(getApplicationContext(),"Please enter password",Toast.LENGTH_LONG).show();
				}
				else if(null==strEmail || strEmail.length()==0 || strEmail.equalsIgnoreCase(" ")){
					Toast.makeText(getApplicationContext(),"Please enter email",Toast.LENGTH_LONG).show();
				}
				else if(checkEmail(strEmail)==YES){
					Toast.makeText(getApplicationContext(),"Please enter a valid email address",Toast.LENGTH_LONG).show();
			    }
				else if(null==strFname || strFname.length()==0 || strFname.equalsIgnoreCase("")){
					...
					{
						AlertDialog ad = new AlertDialog.Builder(RegistrationActivity.this, ProgressDialog.THEME_HOLO_LIGHT).create();
						ad.setTitle(Html.fromHtml("<font color='"+getResources().getColor(R.color.tabDark)+"'>Information</font>"));
	 ...
	 protected Dialog onCreateDialog(int id) {
		...
		private void updateDisplay() {
			 tvDOB.setText(new StringBuilder().append(mYear1).append("-").append(mMonth1+1).append("-").append(mDay1).append(""));
			 
			 strDOB = (tvDOB.getText().toString()).replaceAll(" ","%20");
		     //String dobArr[] = strDOB.split("-");
		     //strDOB = dobArr[2]+"-"+dobArr[0]+"-"+dobArr[1];	
	...
	private class Signup extends AsyncTask<Void, Void, Void> {  
        ...
        protected void onPreExecute() {
        	tempdata.clear();
	        logindata.clear();	        
        	pd = new ProgressDialog(RegistrationActivity.this, ProgressDialog.THEME_HOLO_LIGHT);
        ...
        protected Void doInBackground(Void... args) {    	
			try {
				...
				String url = "http://allmediany.com/service/allmediany-service.svc/getUserSignup/format=json/UserUname="+strUsername+"/UserPwd="+strUserPwd+"/UserEmail="+strEmail+"/UserFname="+strFname+"/UserLname="+strLname+"/CountryID="+strCountryId+"/StateID="+strStateId+"/CityID="+strCityId+"/UserAddr="+strAddr+"/ZipCode="+strZip+"/Phone="+strPhone+"/dob="+strDOB+"/";

				url  = url.replaceAll(" ","%20");

				Log.d("URL","RegistrationActivity: "+url);
	    ...
	    protected void onPostExecute(Void result) {
			...
			if(logindata.get(0).get(CHECK_MSG).equalsIgnoreCase("Success"))
			{
			   ...
			   Log.d("URL",""+logindata.get(0).get(CUST_EMAIL));
			...
			else
			{
				...
				AlertDialog alertDialog = new AlertDialog.Builder(RegistrationActivity.this, ProgressDialog.THEME_HOLO_LIGHT).create();
...
protected void onActivityResult(int requestCode, int resultCode, Intent data) { 
    ...
    private boolean checkEmail(String emailid1) {    			
		boolean YES=EMAIL_ADDRESS_PATTERN.matcher(emailid1).matches();
		return YES;
	}
}